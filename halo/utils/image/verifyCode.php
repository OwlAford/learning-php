...
class securityCoder {  
  ...
  public static $seKey = 'sid.sekey';  
  public static $expire = 60;                // 验证码过期时间（s）  
  ...
  public static function entry() {  
    // 图片宽(px)  
    self::$imageW || self::$imageW = self::$length * self::$fontSize * 1.5 + self::$fontSize * 1.5;
    // 图片高(px)  
    ...
    // 验证码使用随机字体   
    // $ttf = dirname(__FILE__) . 'fonts/' . mt_rand(1, 20) . '.ttf';
    $ttf = ROOTPATH . '/utils/image/fonts/Helvetica.otf';  
  ...
  protected static function _writeCurve() {  
    ...
    $T = mt_rand(self::$imageH * 1.5, self::$imageW * 2);  // 周期  
    $w = (2 * M_PI) / $T;  
             
    $px1 = 0;  // 曲线横坐标起始位置  
    $px2 = mt_rand(self::$imageW / 2, self::$imageW * 0.667);  // 曲线横坐标结束位置       
...
// 验证验证码 
// if (!securityCoder::check(@$_POST['secode'])) { 
//   print 'error secode'; 
// }